...
export const getCategoryGuide = (categoryName) => ({
  ...defaultGuide,
  ...(teaCategoryGuides[categoryName] || {})
})

export const decorateCategories = (categories = []) => categories.map((category) => {
  const guide = getCategoryGuide(category.name)

  return {
    ...category,
    ...guide,
    description: category.description || guide.recommendation,
